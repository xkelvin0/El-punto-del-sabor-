...
window.agregarAlCarrito = function(id, nombre, precio, imagen, cantidad = 1) {
    ...
    if (esPromo2x1) {
        ...
        if (productoExistente) {
            // Si ya existe, incrementar en 1 par (2 unidades)
            productoExistente.cantidad += 2;
        } else {
            // Si no existe, agregar 1 par (2 unidades)
            carrito.push({ 
                id, 
                nombre: nombre, // Nombre original
                precio: precio, // Precio por par (2 unidades)
                precioUnitario: precio, // Guardar el precio unitario para mostrar
                imagen, 
                cantidad: 2, // 2 unidades = 1 par
    ...
    mostrarNotificacion(esPromo2x1 ? 
        `¡Promoción 2x1 aplicada! 2x ${nombre} por S/ ${precio}` : 
...
function mostrarCarrito() {
    ...
    if (btnPagar) {
        btnPagar.removeEventListener('click', manejarClickPagar);
        btnPagar.addEventListener('click', manejarClickPagar);
    }
...
function actualizarCarrito() {
    ...
    carrito.forEach(item => {
        ...
        if (item.esPromo2x1) {
            // Para la promoción 2x1, el precio ya es por par
            const pares = item.cantidad / 2;
            precioFinal = item.precio * pares;
            mostrarPrecioUnitario = item.precio; // Mostrar el precio por par
        } 
        ...
        // Calcular la cantidad a mostrar (pares para promoción 2x1, unidades para el resto)
        const cantidadMostrar = item.esPromo2x1 ? item.cantidad / 2 : item.cantidad;
        const etiquetaCantidad = item.esPromo2x1 ? 'par(es)' : '';
        
        itemElement.innerHTML = `
            <img src="${item.imagen}" alt="${item.nombre}" class="carrito-item-imagen">
            <div class="carrito-item-info">
                <h4>${item.nombre} ${item.esPromo2x1 ? '<span class="etiqueta-promo">(2x1)</span>' : ''}</h4>
                ${item.esPromo2x1 ? 
                  `<p class="precio">S/ ${precioFinal.toFixed(2)} <span class="precio-unitario">(2 x S/ ${mostrarPrecioUnitario.toFixed(2)} c/u)</span></p>` : 
                  `<p class="precio">S/ ${precioFinal.toFixed(2)}</p>`}
                <div class="cantidad-controls">
                    <button class="cantidad-btn" data-action="decrease" data-id="${item.id}" ${item.cantidad <= (item.esPromo2x1 ? 2 : 1) ? 'disabled' : ''}>-</button>
                    <span class="cantidad">${cantidadMostrar} ${etiquetaCantidad}</span>
