...
function actualizarCantidad(cambio) {
  ...
  // Detener cualquier otro evento que pueda estar en cola
  if (window.cantidadUpdateInProgress) return;
  window.cantidadUpdateInProgress = true;
  
  let valor = parseInt(inputCantidad.value) + cambio;
  if (valor < 1) valor = 1;
  inputCantidad.value = valor;
  
  // Permitir el próximo cambio después de un pequeño retraso
  setTimeout(() => {
    window.cantidadUpdateInProgress = false;
  }, 10);
}
...
function cargarProducto() {
  ...
  const contenedorGaleria = document.querySelector('.producto-galeria');
  const contadorProducto = document.querySelector('.producto-contador');
  ...
  setTimeout(() => {
    try {
      if (productoId && productos[productoId]) {
        ...
        // Mover el contador debajo de la galería
        if (contadorProducto && contenedorGaleria && !contadorProducto.dataset.enGaleria && window.innerWidth <= 576) {
          contadorProducto.dataset.enGaleria = 'true';
          contenedorGaleria.appendChild(contadorProducto);
          contadorProducto.classList.add('contador-en-galeria');
        }

        // Mover los botones de acción debajo del contador
        if (botonesAccion && contenedorGaleria && !botonesAccion.dataset.enGaleria && window.innerWidth <= 576) {
          botonesAccion.dataset.enGaleria = 'true';
          contenedorGaleria.appendChild(botonesAccion);
          botonesAccion.classList.add('botones-accion-en-galeria');
        }
      } else {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (btnDisminuir && btnAumentar && inputCantidad) {
    // Eliminar cualquier event listener existente clonando los elementos
    const newDisminuir = btnDisminuir.cloneNode(true);
    const newAumentar = btnAumentar.cloneNode(true);
    const contenedor = btnDisminuir.parentNode;
    
    // Reemplazar los botones con clones para eliminar event listeners
    contenedor.replaceChild(newDisminuir, btnDisminuir);
    contenedor.replaceChild(newAumentar, btnAumentar);
    
    // Agregar los nuevos event listeners
    newDisminuir.addEventListener('click', function(e) {
      e.preventDefault();
    ...
    newAumentar.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopImmediatePropagation();
      actualizarCantidad(1);
      return false;
